...
public class ClientGUI extends JFrame {
		...
		try {
			...
			while (true) {
				...
				if(!chatList.isEmpty()){
					...
					String bip = "src/main/files/Sounds/Game Over.mp3";
					Media hit = new Media(getFileFromResources(bip).toURI().toString());
	...
	public void dealCards(String cards) {
		...
		for (int x = 0; x < cardArray.length; x++) {
			card = newHand[x];
			playersHand[x] = card;
			pic = new ImageIcon("src/main/files/Cards/" + card + ".png");
		...
		try {
			...
			if (swaps > 0) {
				...
				for (int x = 0; x < indexes.size(); x++) {
					...
					ImageIcon pic = new ImageIcon("src/main/files/Cards/" + playersHand[indexes.get(x)] + ".png");
